...
class GqlImporter():
    ...
    def find_spec(cls, name, path, target=None):
        ...
        for filename in gql_file_names:
            for directory in directories:
                gql = pathlib.Path(directory) / filename
                if gql.exists():
                    return ModuleSpec(name, cls(gql))
    ...
    def exec_module(self, module):
        ...
        with open(f"queries.gql", "r") as f:
...
# Add the GQL importer at the end of the list of finders
sys.meta_path.append(GqlImporter)